...
# Sjekk at kolonnen finnes (skal gi TRUE):
"gdp_pc_n3" %in% names(eu_data)

# 1) Filtrer ut NUTS3 i den NUTS1-regionen i Frankrike som skiller seg ut (FR1)
fr1_n3 <- eu_data |>
  filter(nc == "FR", n1 == "FR1") |>
  select(time, n3, gdp_pc_n3) |>
  filter(!is.na(gdp_pc_n3))

# 2) Linjeplot: NUTS3-regioner i FR1 (GDP per capita, NUTS3)
...
```{r}
ggplot(fr_n3, aes(x = time, y = gdp_pc_n3, group = n3, colour = n3)) +
  geom_line() +
  labs(
    x = "Year",
